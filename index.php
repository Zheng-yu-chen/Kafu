...
<style>
    /* 原有樣式保持不變 */
    .top-banner { background-color: #002B5B; color: white; padding: 12px 20px 55px; text-align: center; }
    ...
    .filter-btn { background: white; color: #002B5B; padding: 8px 18px; border-radius: 20px; text-decoration: none; font-size: 14px; box-shadow: 0 4px 10px rgba(0,0,0,0.15); font-weight: bold; }
    .filter-btn.active { background: #002B5B; color: white; }
    ...
    /* --- 3. 💡 美食家助理：絕對定位樣式 --- */
    #ai-assistant-fab {
        position: absolute !important; /* 💡 絕對定位在 .mobile-wrapper 內 */
        bottom: 100px;
        right: 20px;
    ...
    .assistant-header { background: #002B5B; color: white; padding: 12px 15px; display: flex; justify-content: space-between; align-items: center; }
    .assistant-body { padding: 18px; font-size: 14px; }
    .recommend-actions { display: flex; gap: 10px; margin: 15px 0; }
    .btn-action { flex: 1; padding: 10px; border: 1.5px solid #FF8C42; background: white; color: #FF8C42; border-radius: 10px; cursor: pointer; font-weight: bold; font-size: 13px; }
    .filter-options { background: #f9f9f9; padding: 12px; border-radius: 12px; }
    .btn-submit { width: 100%; background: #002B5B; color: white; border: none; padding: 10px; border-radius: 10px; margin-top: 10px; font-weight: bold; cursor: pointer; }
    #recommend-result { margin-top: 15px; padding: 12px; background: #FFF3EB; border-radius: 12px; border-left: 4px solid #FF8C42; }
    .result-name { font-weight: bold; color: #002B5B; display: block; }
    .btn-go { display: inline-block; margin-top: 8px; color: #FF8C42; text-decoration: none; font-weight: bold; }
</style>
    ...
    <div id="assistant-card" class="assistant-card">
        ...
        <div class="assistant-body">
            ...
            <div id="filter-section" style="display:none;" class="filter-options">
                <label class="checkbox-item"><input type="checkbox" class="pref-check" value="low_cal"> 🥗 低卡優先 (< 500k)</label>
                <label class="checkbox-item"><input type="checkbox" class="pref-check" value="high_pro"> 🥩 高蛋白需求</label>
                <label class="checkbox-item"><input type="checkbox" class="pref-check" value="is_veg"> 🥬 我想吃素</label>
...
function fetchRecommend(mode) {
    ...
    resDiv.innerHTML = `
        ...
        <p style="font-size: 12px; margin: 5px 0;">於 ${data.restaurant} | ${data.calories} kcal</p>
        
        <a href="restaurant_detail.php?r_id=${data.r_id}" class="btn-go">前往餐廳看看 ❯</a>
    `;
        } else {
            resDiv.innerHTML = "<p style='color:#999;'>暫時找不到符合條件的餐點...🥺</p>";
        }
    });
}
</script>
